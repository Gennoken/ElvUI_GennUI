...
-- if GennUIDB.TooltipIcons ~= true or IsAddOnLoaded("TooltipItemIcon") then return end
local VERSION = 1.95
local VERSIONINFO = "X-Release"

local NEWTOOLTIPS = (C_TooltipInfo and TooltipUtil and TooltipDataProcessor and TooltipDataProcessor.AddTooltipPostCall) and
true or false

--------------------------------------------------------------------------------
-- SPECIAL HANDLING: SECRETS
--------------------------------------------------------------------------------
local issecretvalue = issecretvalue or
function() end                                        -- where 'issecretvalue' is not defined use the empty function (always returns nil) instead
	...
	GetMerchantItemInfo = function(index)
		...
		if info then
			return info.name, info.texture, info.price, info.stackCount, info.numAvailable, info.isPurchasable,
				info.isUsable, info.hasExtendedCost, info.currencyID
		end
	end
end
...
local function HookMacro(frame, datatable)
	...
	local info = datatable.lines
	[1]                          -- Assume lines always exists at this point, and that the info we need is in lines[1]
...
local function SlashGlobalActions(command, extra)
	...
	elseif command == "fade" then
		...
		elseif saved.mode == "background" then
			saved.background.tintr, saved.background.tintg, saved.background.tintb = extranumber, extranumber,
				extranumber
			for _, data in pairs(IconDataTable) do
